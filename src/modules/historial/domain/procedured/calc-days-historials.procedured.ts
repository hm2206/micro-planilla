...
export class CalcDaysHistorialsProcedured extends DatabaseProcedured {
  ...
  public query() {
    return (
      ...
      INNER JOIN (
        SELECT h.id, 
        IFNULL(DATEDIFF(cont.terminationDate, CONCAT(cro.\`year\`, '-', cro.\`month\`, '-01')), 0) + 1 as calcDays,
        cro.numberOfDays
      ...
      SET his.days = IF(up.calcDays > up.numberOfDays, up.numberOfDays, IF(up.calcDays < 0, 0, up.calcDays));
      `
    )
  }
}